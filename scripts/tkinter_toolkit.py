...
class CmdTerminal(tk.Frame):
    ...
    def execute_command(self, event=None):
        ...
        try:
            ...
            process.kill()
            process.communicate()
            self.write_output("⏱️ Comando cancelado por timeout (60s).\n")
        except Exception as e:
            self.write_output(f"Error al ejecutar: {e}\n")
    
    def run_as_admin(self):
        """Ejecuta un comando con privilegios elevados mediante PowerShell (Start-Process -Verb RunAs)"""
        cmd = self.entry.get().strip()
        if not cmd:
            self.write_output("Escribe un comando primero.\n")
            return
        try:
            # Usar PowerShell con Start-Process -Verb RunAs para elevación limpia
            ps_cmd = (
                f'Start-Process cmd.exe -ArgumentList "/k title Terminal Admin ARKE && '
                f'{cmd} && pause" -Verb RunAs'
            )
            subprocess.Popen(
                ['powershell', '-Command', ps_cmd],
                creationflags=subprocess.CREATE_NO_WINDOW
            )
            self.write_output(f"🔓 Elevado (nueva ventana): {cmd}\n")
        except Exception as e:
